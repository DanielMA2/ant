...
void Read(){
TFile *data = new TFile("unpol_brems_Primakoff_gp_ppi0_100kEvs_ant_output.root");
...
TCanvas *c1 = new TCanvas("c1","unpolBeam_basic_plots");
c1->Divide(3);
c1->cd(1);
gStyle->SetOptStat(1);
...
TCanvas *c2 = new TCanvas("c2","unpolBeam_Polar_angle_hists");
c2->Divide(3);
c2->cd(1);
gStyle->SetOptStat(1);
...
line->Draw();
line2->Draw();
c2->SaveAs("new_hists/unpolBeam_Polar_angle_hists.pdf");

//Plotting & saving azimuth angle histograms

TCanvas *c3 = new TCanvas("c3","unpolBeam_Phi_angle_hists");
c3->Divide(3);
c3->cd(1);
...
c3->SaveAs("new_hists/unpolBeam_Azimuth_angle_hists.pdf");

//Plotting & saving particle invariant mass histograms

TCanvas *c4 = new TCanvas("c4","unpolBeam_IM_hists");
c4->Divide(2);
c4->cd(1);
...
TF1 *func2 = new TF1("fit2","gaus",850,1000);
func2->SetParameter(1,130);
func2->SetLineColor(kGreen);
gStyle->SetOptFit();
h_ProtonIm->Fit(func2,"R");
h_ProtonIm->Draw();
c4->SaveAs("new_hists/unpolBeam_Im_hists.pdf");

//Plotting & saving particle energy vs theta histograms

TCanvas *c5 = new TCanvas("c5","unpolBeam_Energy_Theta_hists");
c5->Divide(2,2);
c5->cd(1);
...
c5->SaveAs("new_hists/unpolBeam_EvsAngles_hists.pdf");
...
h_InitialBeamE->Draw();
c6->SaveAs("new_hists/unpolBeam_InitialBeamE.pdf");
